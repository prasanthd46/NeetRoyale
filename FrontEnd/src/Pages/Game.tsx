...
const Game = () => {
    ...
    if (round == 1) {
      ...
      socket?.on("powerUps", (response) => {
        if (response) {
          ...
          console.log("powerUps got error");
        }
      });
  ...
  return (
    <div className="bg-[#8e70f7] px-30 py-20 h-screen font-[Roboto]">
      <div className="bg-white flex items-center p-1 flex-col rounded-xl h-full justify-center">
        <div className=" px-40 py-5 w-full rounded-full ">
          <div className="p-2 flex h-[100px] justify-between">
            <div className="text-xl font-extrabold">Round-{round}</div>
            {round == 2 && (
              <div className="flex flex-col">
                {showPlayers && (
                  <div className="flex gap-2 p-1">
                    ...
                    {targetId &&
                      targetId.map((player, index) => (
                        <div
                          className="hover:bg-gray-200 rounded-full text-[10px] flex justify-center items-center px-1 font-bold text-amber-600 hover:cursor-pointer"
                          key={index}
                          onClick={() => {
                ...
                <div className="flex gap-4 text-xl">
                  <div
                    className={`flex items-center  hover:bg-amber-100 hover:rounded-full hover:bg-opacity-150 ${
                      powerUps.includes("doublePoints")
                  ...
                  <div
                    className={`flex items-center  hover:bg-amber-100 hover:rounded-full hover:bg-opacity-150 ${
                      powerUps.includes("freezeOpponent")
                        ? "opacity-100 cursor-pointer"
                        : "opacity-50"
                    }`}
                    onClick={
            ...
            <div className="p-10  w-[50%]  max-h-[500px] bg-[#fcfbff] rounded-xl flex flex-col shadow-md">
              ...
              <div className="text-[20] ">
                {(round == 1
                  ...
                  <div
                    key={index}
                    className={`pr-5 p-1 flex justify-between ${
                      selected === index
                        ? " bg-white h-[42px] rounded-r-xl shadow-xl"
                        : "border-1 rounded border-gray-300"
                    } mt-5`}
                    onClick={() => {
                      if (round == 1) {
                        setSelected(index);
                  ...
                  >
                    <div className="flex justify-center">
                      <div
                        className={`w-1 h-full flex mr-6 ${
                          selected === index ? "bg-[#8e70f7]" : "bg-transparent"
                        }`}
                      ></div>
                    ...
                    <input
                      type="radio"
                      className="form-radio accent-[#8e70f7] bg-transparent"
          ...
          <div className="flex justify-center gap-10 mt-10">
            {round === 1 ? (
              <>
                <div
                  className="roundedd-lg p-2 bg-teal-200 cursor-pointer"
                  onClick={() => {
                ...
                {currentQuestionIndex != questions.length - 1 ? (
                  ...
                  <div
                    className="rounded-lg p-2 bg-teal-200 cursor-pointer"
                    onClick={() => {
                      submission();
                      setSelected(-1);
